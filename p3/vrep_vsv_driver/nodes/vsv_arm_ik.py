...
class VSVArmIK:
    def __init__(self):
        ...
        self.psub = rospy.Subscriber('~position_stamped', PointStamped, self.point_stamped_cb)
    ...
    def point_stamped_cb(self,value):
        #print(value)
        self.listener.waitForTransform('/%s/ArmPan'%(self.name),value.header.frame_id,value.header.stamp,
                rospy.Duration(1.0))
        armpoint = self.listener.transformPoint('/%s/ArmPan'%(self.name),value)
        #print(Point(-armpoint.point.y,armpoint.point.x,armpoint.point.z))
        self.point_cb(Point(-armpoint.point.y,armpoint.point.x,armpoint.point.z))
