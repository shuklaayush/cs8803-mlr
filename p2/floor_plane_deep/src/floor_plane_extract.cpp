...
protected:
    ...
    boost::shared_ptr<message_filters::Subscriber<sensor_msgs::PointCloud2> >
        pclSub;
    typedef message_filters::TimeSynchronizer<sensor_msgs::PointCloud2,
                                              sensor_msgs::Image>
        TS;
    ...
    typedef enum { UNUSABLE, UNTRAVERSABLE, TRAVERSABLE } ThumbType;

    ThumbType check_thumb(const cv::Mat_<cv::Vec3b>& thumb,
                          const cv::Mat_<float>& thumb_z) {
        // TODO: Modify this function to take a decision on the traversability
        // of the patch of ground corresponding to this image.
        float min_z = INFINITY;
        float max_z = -INFINITY;
        for (int r = 0; r < thumb_z.rows; r++) {
            for (int c = 0; c < thumb_z.cols; c++) {
                if (std::isnan(thumb_z(r, c))) {
                    // ignore this point, it has not been observed from the
                    // kinect
                    continue;
                }
                if (thumb_z(r, c) < min_z) {
                    min_z = thumb_z(r, c);
                } else if (thumb_z(r, c) > max_z) {
                    max_z = thumb_z(r, c);
                }
            }
        }
        auto diff_z = max_z - min_z;
        if (diff_z > 0) {
            if (diff_z < THRESH) {
                return TRAVERSABLE;
            } else {
...
protected:  // ROS Callbacks
    ...
    void sync_callback(const sensor_msgs::PointCloud2ConstPtr pcl_msg,
                       const sensor_msgs::ImageConstPtr img_msg) {
        if (!has_info) return;

        tf::StampedTransform tfw;
        listener_.waitForTransform(base_frame_, world_frame_,
                                   pcl_msg->header.stamp, ros::Duration(1.0));
        listener_.lookupTransform(base_frame_, world_frame_,
                                  pcl_msg->header.stamp, tfw);
        // Check if we moved
        if ((hypot(last_pose.x - tfw.getOrigin().x(),
                   last_pose.y - tfw.getOrigin().y()) < min_displacement_) &&
            (fabs(remainder(last_pose.theta - tf::getYaw(tfw.getRotation()),
                            2 * M_PI)) < min_rotation_)) {
            return;
        }
        ...
        listener_.waitForTransform(base_frame_, pcl_msg->header.frame_id,
                                   pcl_msg->header.stamp, ros::Duration(1.0));
        pcl_ros::transformPointCloud(base_frame_, pcl_msg->header.stamp,
                                     pc_sensor, pcl_msg->header.frame_id,
                                     pc_base, listener_);
        ...
        for (int r = 0; r < img.rows; r += thumb_size_) {
            if (r + thumb_size_ > img.rows) {
                continue;
            }
            for (int c = 0; c < img.cols; c += thumb_size_) {
                // printf("r %d c %d\n",r,c);
                if (c + thumb_size_ > img.cols) {
                    continue;
                }
                ...
                // printf("type : %d\n",type);
                if ((type == TRAVERSABLE) &&
                    (traversable_counter_ >= (unsigned)max_image_per_type_)) {
                    // We have enough images of this type
                    type = UNUSABLE;
                }
                if ((type == UNTRAVERSABLE) &&
                    (untraversable_counter_ >= (unsigned)max_image_per_type_)) {
                    // We have enough images of this type
                    type = UNUSABLE;
                }
                if (type != UNUSABLE) {
        ...
        printf("Image counter at %ld (%ld / %ld)\n", image_counter_,
               traversable_counter_, untraversable_counter_);
...
public:
    FloorPlaneExtract() : nh_("~"), it_(nh_) {
        ...
        info_sub_ = nh_.subscribe(
            "info", 1, &FloorPlaneExtract::calibration_callback, this);
        // scan_sub_ =
        // nh_.subscribe("scans",1,&FloorPlaneExtract::pc_callback,this);
        // image_sub_ = it_.subscribe<FloorPlaneExtract>("image",1,
        // &FloorPlaneExtract::image_callback,this,transport);

        // Created a synchronized subscriber for point cloud and image. The
        // need for pointer is suspicious, but it works this way.
        imgSub.reset(new message_filters::Subscriber<sensor_msgs::Image>(
            nh_, "image", 1));
        pclSub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(
            nh_, "pointcloud", 1));
        sync.reset(new TS(*pclSub, *imgSub, 50));
        sync->registerCallback(
            boost::bind(&FloorPlaneExtract::sync_callback, this, _1, _2));
    }
};
